...
export type NLoginType = NLoginBunker | NLoginExtension | NLoginOther;
...
type NLoginBase<T extends string, D> = {
  id: string;
  type: T;
  pubkey: string;
  createdAt: string;
...
export class NLogin<T extends string, D> implements NLoginBase<T, D> {
  ...
  static async fromExtension(): Promise<NLogin<"extension", null>> {
