...
<template>
  <div v-bind="$attrs">
    <template v-if="activePlan.hasActivePlan">
      ...
      >
            ...
            >
              <div class="flex flex-col gap-5">
                <div class="flex items-start gap-4">
                  <USkeleton
                    class="rounded-full h-12 w-12 sm:h-14 sm:w-14 shrink-0"
                  />
                  <div class="flex-1 space-y-3 w-full">
                    <div
                      class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between w-full"
                    >
                      <div class="space-y-2">
        ...
        <template #acks>
          ...
          <div v-else-if="acks.length > 0" class="space-y-4 mt-4">
            <AppAckCard v-for="ack in acks" :key="ack.id" :ack="ack" />
          </div>
          <div
            v-else
            class="flex flex-col items-center justify-center py-16 text-center space-y-3"
      ...
      <AppFeedRawModal v-model="isRawModalOpen" :signal="selectedSignal" />
    </template>
  </div>
</template>
