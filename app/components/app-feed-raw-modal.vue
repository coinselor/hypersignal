...
<template>
  ...
  >
    <template #body>
      <div
        class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-md overflow-hidden"
      >
        <div
        ...
        <div class="overflow-auto max-h-[calc(100vh-200px)] p-4">
          <pre
            v-if="signal"
            class="text-sm"
          ><code ref="codeBlock" class="language-json">{{ JSON.stringify(signal.rawEvent, null, 2) }}</code></pre>
        </div>
      </div>
    </template>
